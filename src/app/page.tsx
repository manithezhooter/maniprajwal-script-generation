...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Read auth error from URL query parameter if present (e.g. from Google Callback redirect)
    if (typeof window !== "undefined") {
      const params = new URLSearchParams(window.location.search);
      const errorParam = params.get("auth_error");
      if (errorParam) {
        let friendlyMessage = errorParam;
        if (errorParam === "google_credentials_missing") {
          friendlyMessage = "Google Authentication is not configured on the server.";
        } else if (errorParam === "token_exchange_failed") {
          friendlyMessage = "Failed to communicate with Google authentication servers.";
        } else if (errorParam === "userinfo_fetch_failed") {
          friendlyMessage = "Failed to retrieve user info from Google.";
        } else if (errorParam === "email_not_provided") {
          friendlyMessage = "Google account did not provide an email address.";
        } else if (errorParam === "internal_server_error") {
          friendlyMessage = "An internal server error occurred during Google sign in.";
        } else if (errorParam === "no_code_provided") {
          friendlyMessage = "Authentication code was not provided.";
        }
        setAuthError(friendlyMessage);
        
        // Clean up the URL query parameter so it doesn't persist on page refresh
          ...
          >
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-400 mt-2">
                Generate high-conversion video scripts instantly.
              </p>
            </div>
            ...
            <form onSubmit={handleAuth} className="space-y-5">
              ...
              <button
                ...
                className="secondary-btn magnetic-btn w-full flex items-center justify-center gap-3"
              >
